...
export function ProductCard({ stock }: { stock: NearestStock }) {
  return (
    <div className="group relative rounded-xl overflow-hidden border-muted border shadow-xl">
      ...
      <Image src={stock.product.images[0].imageUrl} width={300} height={300} alt={stock.product.title} className="group-hover:scale-110 w-full h-60 object-contain transition-transform duration-500 ease-in-out transform hover:scale-110 " />
      <div className="p-4 bg-background flex flex-col justify-between h-auto">
        <h3 className="text-lg font-semibold max-h-14 truncate">{stock.product.title}</h3>
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-center mt-2">
            <p className="text-muted-foreground text-sm line-through" suppressHydrationWarning>
