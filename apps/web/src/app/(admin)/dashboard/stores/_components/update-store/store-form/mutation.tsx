...
export const useUpdateStore = (handleClose: () => void, store: Store) => {
  ...
  >({
    mutationFn: async (formData: FormData) => {
      try {
        const data = await fetchAPI(`${env.NEXT_PUBLIC_BASE_API_URL}/stores/${store.id}`, {
          method: 'PATCH',
          body: formData,
        });
    ...
    onSuccess: (data) => {
      router.refresh()
      toast(`${data.store.name} store has been updated!`, { duration: 4000 });
      handleClose();
