...
const AwaitingPayment: React.FC<OrderItemProps> = ({ order }) => {
  ...
  return (
    <div className="border rounded-lg p-4 mb-4 shadow-md">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div className="flex-1">
          ...
          <div className="text-sm text-gray-600 mb-1">
            <span className="text-gray-700">Date Placed:</span> {new Date(order.updatedAt).toLocaleDateString()}
          </div>
          <div className="text-sm text-gray-600 mb-1">
            <span className="text-gray-700">Total Amount:</span> {formatCurrency(order.totalPayment)}
          </div>
        </div>
      </div>
          ...
          <div className="flex-1">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-600 text-sm">{item.stock?.product?.description}</p>
              </div>
              <p className="text-lg font-medium">{formatCurrency(item.isPack ? item.stock?.product?.packPrice : item.stock?.product?.price)}</p>
            </div>
      ...
      <div className="mt-4">
        <Link className="bg-primary text-white rounded-xl p-3 mr-3" href={order.paymentLink ? order.paymentLink : `/orders/payment-proof/${order.id}`}>
          Pay
        </Link>
        <Button type="button" className="rounded-xl" onClick={handleCancelOrder}>
          Cancel
        </Button>
      </div>
    </div>
