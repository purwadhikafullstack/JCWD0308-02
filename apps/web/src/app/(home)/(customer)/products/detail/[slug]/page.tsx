  ...
  const handleAddToCart = () => {
    ...
    try {
      ...
      const availableStock =
        product?.stock?.reduce((total, stock) => total + stock.amount, 0) || 0;
            ...
            <div className="w-full md:w-1/2 md:pl-8 p-4">
              ...
              {isPack ? (
                <p className="text-2xl font-bold mb-4 text-primary">
                  <del className="text-xs font-semibold text-gray-500 line-through">
                    {formatCurrency(product.packPrice || 0)}
                  </del>{' '}
                  {formatCurrency(
                    product.discountPackPrice || product.packPrice || 0,
                  )}
                </p>
              ) : (
                <p className="text-2xl font-bold mb-4 text-primary">
              ...
              <div className="flex space-x-4">
                <Button
                  ...
                  disabled={availableStock === 0 || !isServiceAvailable}
              ...
              <div className="mt-6 space-y-2">
                {product.bonus !== undefined && (
                  <div className="flex items-center text-gray-600">
                    ...
                    <span>Bonus: {product.bonus}</span>
                  </div>
                )}
                {product.minOrderItem !== undefined && (
                  <div className="flex items-center text-gray-600">
                    <CheckCircle size={16} className="mr-2" />
                    <span>Min Order: {product.minOrderItem}</span>
                  </div>
                )}
