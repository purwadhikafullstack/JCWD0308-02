...
export default function Cart() {
  ...
  function calculateSubtotal(items: CartItemType[]) {
    return items.reduce((acc, item) => {
      ...
      if (selectedItem[key]) {
        const price = item.isPack ? item.stock?.product?.discountPackPrice || item.stock?.product?.packQuantity : item.stock?.product?.discountPrice || item.stock?.product?.price;
