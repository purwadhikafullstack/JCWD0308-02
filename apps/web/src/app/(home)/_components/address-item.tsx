...
export default function AddressItem({
  ...
  return (
    <Card className={cn("flex items-center justify-between", address.id === data?.address?.id ? "border-primary" : "")}>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant={"ghost"} size={"icon"} className="h-3 w-3 text-destructive">
                <Trash />
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Are you sure want to delete {address.labelAddress} address?</AlertDialogTitle>
                <AlertDialogDescription>This action cannot be undone. This will permanently delete {address.labelAddress} from our servers.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
