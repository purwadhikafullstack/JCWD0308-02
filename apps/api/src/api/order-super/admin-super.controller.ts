...
import { OrderSuperService } from "./admin-super.service.js";
...
export class OrderSuperController {
  getAllOrders: ICallback = async (req, res, next) => {
    try {
      ...
      const { orders, totalCount } = await OrderSuperService.getAllOrders(page, perPage, res);
  ...
  confirmPayment: ICallback = async (req, res, next) => {
    try {
      ...
      const result = await OrderSuperService.confirmPayment(confirmPaymentRequest, res);
