...
export class AuthHelper {
  ...
  static getGitHubUserData = (token: string) => {
    return fetch(`${GITHUB_OAUTH_URL}/user`, { headers: { Authorization: `Bearer ${token}` } })
  }

  static getGoogleUserData = async (token: string) => {
    return (await fetch(`${GOOGLE_OAUTH_URL}/v1/userinfo`, { headers: { Authorization: `Bearer ${token}` } })).json()
  }

  static getGitHubUserEmail = (token: string) => {
    return fetch(`${GITHUB_OAUTH_URL}/user/emails`, { headers: { Authorization: `Bearer ${token}` } })
  }

  static getGitHubUser = async (token: string): Promise<ICreateUserByGitHub> => {
