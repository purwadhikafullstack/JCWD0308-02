...
export class CartService {
  ...
  static updateCart = async (req: UpdateCartRequest, res: Response) => {
    ...
    const stock = await prisma.stock.findUnique({ where: { id: patchCart.stockId! } });
    if (!stock) throw new ResponseError(400, "Stock not found!");

    const cartItem = await prisma.orderItem.findUnique({ where: { id: patchCart.cartItemId! }, select: { id: true } });
  ...
  static getCart = async (userId: string, res: Response) => {
    ...
    const carts = await prisma.orderItem.findMany({
      where: { userId, orderItemType: OrderItemType.CART_ITEM, isDeleted: false },
    ...
    if (itemsToDelete.length > 0) {
      await prisma.orderItem.updateMany({ where: { id: { in: itemsToDelete } }, data: { isDeleted: true, deletedAt: new Date() } });
    }
    const updatedCarts = await prisma.orderItem.findMany({ where: { userId, orderItemType: OrderItemType.CART_ITEM, isDeleted: false }, include: { stock: { include: { product: { include: { images: true } } } } } });
    return updatedCarts;
  };
