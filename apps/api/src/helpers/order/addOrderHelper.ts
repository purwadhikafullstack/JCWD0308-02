...
const validateUserVoucher = async (userVoucherId: string) => {
  const userVoucher = await prisma.userVoucher.findUnique({ where: { id: userVoucherId }, include: { voucher: true } });
...
export const applyVoucherDiscount = async (userVoucherId: string, totalPrice: number, shippingCost: number, cartItemsLength: number) => {
  ...
  await prisma.userVoucher.update({ where: { id: userVoucher.id }, data: { isUsed: true } });
  return { discountProducts, discountShippingCost };
};
...
export const updateOrderItemsAndStock = async (updatedCartItem: any[], orderId: any) => {
  await prisma.$transaction([
    ...
    ...updatedCartItem.map((item: any) => prisma.stock.update({ where: { id: item.stockId }, data: { amount: { decrement: item.quantity } } })),
    ...updatedCartItem.map((item: any) =>
      prisma.stockMutation.create({
        data: { stockId: item.stockId, mutationType: "ORDER", amount: item.quantity, orderId: orderId },
      }),
    ),
  ]);
};
