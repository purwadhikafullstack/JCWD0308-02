...
const validateVoucherUsage = (
  voucher: any,
  totalPrice: number,
  cartItemsLength: number,
) => {
  if (
    (voucher.minOrderPrice && totalPrice < voucher.minOrderPrice) ||
    (voucher.minOrderItem && cartItemsLength < voucher.minOrderItem)
  ) {
...
const getDiscountAmount = (voucher: any, amount: number) => {
  return voucher.discountType === 'FIXED_DISCOUNT'
    ? voucher.fixedDiscount || 0
    : (amount * (voucher.discount || 0)) / 100;
};
...
export const applyVoucherDiscount = async (
  ...
  const { discountProducts, discountShippingCost } = calculateDiscount(
    voucher,
    totalPrice,
    shippingCost,
    cartItemsLength,
...
export const applyDiscounts = async (
  ...
  if (!orderRequest.userVoucherId)
    return { discountProducts: 0, discountShippingCost: 0 };
  return await applyVoucherDiscount(
...
export const updateOrderItemsAndStock = async (
  ...
  const updates = updatedCartItem.map(async (item) => {
    const decrementValue = item.quantity;
    const amountValue = item.quantity;

    console.log(
      `Updating Order Item ${item.id}: orderId=${orderId}, decrement=${decrementValue}, amount=${amountValue}`,
    );
